	...
	if ($attach) {
		for ($i = 0; $i < count($attach['name']); $i++) {
			if ($attach['error'][$i] != 4) {
				...
				$cdnFilePath = uploadQiNiu($file_info['file_path']);
				
				if($cdnFilePath===false){
				...
				if (file_exists($file_info['file_path'])) {
					@unlink($file_info['file_path']) or emMsg("删除附件失败!");
				}
				...
				$query = sprintf($query, $logid, $file_info['file_name'], $file_info['size'], $cdnFilePath, time(), $file_info['width'], $file_info['height'], $file_info['mime_type']);
				$DB->query($query);
				$aid = $DB->insert_id();
